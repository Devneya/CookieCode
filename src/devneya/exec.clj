...
(defn exec-code
  ...
  ([config]
   ...
   (try
     (bp/shell {:err (:DENO_ERROR_FILENAME config)} (str (get-deployctl-command) " deploy --token=" (:DENO_DEPLOY_TOKEN config) " --project=" (:DENO_PROJECT config) " " (:CODE_FILENAME config)))
